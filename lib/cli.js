...
var init = function (domain, options) {
    ...
    if (!fs.existsSync(awsConfigPath))
    {
        return async.series([
            ...
            initWrapper(domain, config);
        });
        
        return;
    }
    
    return initWrapper(domain, readConfig(awsConfigPath));
};
...
var initWrapper = function (domain, config, callback) {
    caisson.init({
        domain: domain,
        awsConfig: config
    }, function(err){
